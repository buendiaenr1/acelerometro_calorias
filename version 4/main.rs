...
impl Default for MyApp {
    fn default() -> Self {
        ...
        let vo2_1:f64 = met * 3.5;
        let vo2_2:f64 = met2 * 3.5;
        let kcal_min_1 = met * 0.0175 * 70.0;
        let kcal_min_2 = met2 * 0.0175 * 70.0;
...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.separator();
            ui.label(">>>>> Calorías vía aceleraciones ");
                ...
                if ui.button("Calcular").clicked() {
                    ...
                    self.kcal_min_1 = self.met * 0.0175 * self.peso;
                    self.kcal_min_2 = self.met2 * 0.0175 * self.peso;
                }
                ...
                ui.label(format!("MET asignado: {:.1} (personas con cardiopatias trabajar de 3 a 5 METs, si no presentan sintomas adversos)", self.met));
                ui.label(format!("MET2 asignado: {:.1}", self.met2));
                ui.label(format!("VO2 estimado 1: {:.1}", self.vo2_1));
                ui.label(format!("VO2 estimado 2: {:.1}", self.vo2_2));
                ui.label(format!("Kcal/min estimado 1: {:.1}", self.kcal_min_1));
                ui.label(format!("Kcal/min estimado 2: {:.1}", self.kcal_min_2));
            ...
            ui.separator();
            ui.separator();
            if ui.button(">>>>> Calorías vía frecuencia cardiaca ").clicked(){
                // Comando a ejecutar: "eq.exe < fc.csv > salida.txt"
                    let status = Command::new("cmd")
                    .args(&["/C", "eq.exe < fc.csv > salida.txt"])
                    .status()
                ...
                if status.success() {
                    ...
                    eprintln!("El comando falló con el código de salida: {:?}", status.code());
                }
            }
        });
    }
